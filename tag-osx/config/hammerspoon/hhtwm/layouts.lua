...
return function(hhtwm)
  ...
  layouts["main-top"] = function(window, windows, screen, index, layoutOptions)
    if #windows == 1 then
      return layouts["monocle"](window, windows, screen)
    end
    ...
    if index == 1 then
      frame.x = frame.x + margin / 2
      frame.y = frame.y + margin / 2
      frame.w = insetFrame.w - margin
      frame.h = insetFrame.h * layoutOptions.mainPaneRatio - margin
    else
      local divs = #windows - 1
  ...
  layouts["tabbed-top"] = function(window, windows, screen, index, layoutOptions)
    if #windows == 1 then
      return layouts["monocle"](window, windows, screen)
    end
